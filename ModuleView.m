...
-(id)initWithFrame:(CGRect)frame{
  ...
  NSString *lastUpdateCheckLabelString = NSLocalizedBundleString(@"UPDATE_LAST_CHECKED_NEVER", nil);
...
-(void)loadUp{
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
	...
	NSString *myString = NSLocalizedBundleString(@"INSTALL_BUTTON_LABEL", nil);
...
if(self.onlineDict){ //This should exist if it is even there?
  if([self.onlineDict objectForKey:@"versionCodeName"]){
    NSString *codeNamelabelString = NSLocalizedBundleString(@"UPDATE_LABEL_WITH_CODE_NAME", nil);
    ...
    NSString *updateLabelString = NSLocalizedBundleString(@"UPDATE_LABEL", nil);
  ...
  if([self.onlineDict objectForKey:@"lastChecked"]){
      NSString *lastUpdateCheckLabelString = NSLocalizedBundleString(@"UPDATE_LAST_CHECKED", nil);
      ...
      if([[self.moduleDict objectForKey:@"updatesAvailable"] intValue] > 0){
         ...
         [self.installUpdatesButton setTitle:[NSString stringWithFormat:NSLocalizedBundleString(@"INSTALL_BUTTON_LABEL", nil), [self.moduleDict objectForKey:@"updatesAvailable"]] forState:UIControlStateNormal];
        }
        else{
            [self.installUpdatesButton setTitle:[NSString stringWithFormat:NSLocalizedBundleString(@"1INSTALL_BUTTON_LABEL", nil), [self.moduleDict objectForKey:@"updatesAvailable"]] forState:UIControlStateNormal];
        }
      }
      else{
        self.updateNameLabel.backgroundColor = [UIColor colorWithRed:0.0f green:0.0f blue:0.0f alpha:0.50];
...
-(void)continuePressed{
  [self.setupInstallController continuePressed];
}
...
-(void)hideInstallScreen{
    ...
    //FIXME: For this use case, we don't want to be able to hide the install screen.
...
[UIView animateWithDuration:0.25
         ...
         self.blurredSnapshotView.userInteractionEnabled = FALSE;
 if(self.setupInstallController) [self.setupInstallController reshowInstallStartViews];
