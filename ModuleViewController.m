...
-(void)continuePressed{
  [moduleView continuePressed]; //Why I decided to route through here I don't know. Bad programming Grayd00r. Bad programming. 
}
...
-(void)installPressed{
  ...
  if(moduleView.installView.updatesToInstall > 1){
    updateText = [NSString stringWithFormat:NSLocalizedBundleString(@"INSTALL_UPDATE_ALERT_BODY", nil), [moduleView.moduleDict objectForKey:@"updatesAvailable"], [moduleView.moduleDict objectForKey:@"displayName"]];
  }
  else{
    updateText = [NSString stringWithFormat:NSLocalizedBundleString(@"INSTALL_1UPDATE_ALERT_BODY", nil), [moduleView.moduleDict objectForKey:@"updatesAvailable"], [moduleView.moduleDict objectForKey:@"displayName"]];
  }
  UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSString stringWithFormat:NSLocalizedBundleString(@"INSTALL_UPDATE_ALERT_TITLE", nil), [moduleView.moduleDict objectForKey:@"displayName"]]
                          message:updateText
                          delegate:self 
                          cancelButtonTitle:NSLocalizedBundleString(@"INSTALL_UPDATE_ALERT_CANCEL", nil)
                          otherButtonTitles:NSLocalizedBundleString(@"INSTALL_UPDATE_ALERT_OKAY", nil), nil];
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section 
{
  ...
  if (section == 0){
     if([[[self.module valueForKey:@"moduleDict"] objectForKey:@"updatesAvailable"] intValue] > 0){
				sectionTitle = NSLocalizedBundleString(@"MODULE_UPDATES_SECTION_TITLE", nil);
        headerView.backgroundColor = [UIColor colorWithRed:0.0f green:0.0f blue:0.25f alpha:0.90];
			}
			else{
				if([self.module valueForKey:@"localComponentsList"]){
					sectionTitle = NSLocalizedBundleString(@"MODULE_INSTALLED_SECTION_TITLE", nil);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ComponentEntryCell *cell = [tableView cellForRowAtIndexPath:indexPath];
  	//NSString *alertString = NSLocalizedBundleString(@"COMPONENT_ALERT_BODY_TEXT", nil);
//alertString = [NSString stringWithFormat:alertString, cell.componentID];
//UIAlertView *alert = [[UIAlertView alloc] initWithTitle:cell.componentTitle message:alertString delegate:self cancelButtonTitle:NSLocalizedBundleString(@"CLOSE_COMPONENT_ALERT_LABEL", nil) otherButtonTitles:nil];
