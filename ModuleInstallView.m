...
[closeButton setBackgroundImage:[[UIImage imageNamed:@"ArrowDown.png"] tintedImageUsingColor:[UIColor blackColor] alpha:1.0]
...
restartButton.frame = CGRectMake((self.frame.size.width / 2) - 120, self.frame.size.height - 70, 240, 60);
 ...
 [restartButton setTitle:[NSString stringWithFormat:NSLocalizedBundleString(@"RESTART_BUTTON_LABEL", nil)] forState:UIControlStateNormal];
...
[restartButton setTitle:NSLocalizedBundleString(@"CONTINUE_SETUP", nil) forState:UIControlStateNormal];
[restartButton setTitleColor:[UIColor colorWithRed:0.0 green:122.0/255.0 blue:1.0 alpha:1.0] forState:UIControlStateNormal];
[restartButton setTitleColor:[UIColor colorWithRed:0.0 green:50.0/255.0 blue:200.0/255.0 alpha:1.0] forState:UIControlStateHighlighted];
restartButton.frame = CGRectMake((frame.size.width / 2) - 160,frame.size.height - 60, 320, 40);
restartButton.font = [UIFont fontWithName:@"Arial" size:24];
restartButton.alpha = 0.0; //Only show this when an OTA option has been chosen!
[restartButton setBackgroundImage:nil forState:UIControlStateNormal];
[self addSubview:restartButton];
...
[hideLogButton setBackgroundImage:[[UIImage imageNamed:@"ArrowDown"] tintedImageUsingColor:[UIColor blackColor] alpha:1.0]
                            forState:UIControlStateNormal];
...
[showLogButton setBackgroundImage:[[UIImage imageNamed:@"Changelog_v7"] tintedImageUsingColor:[UIColor blackColor] alpha:1.0]
                            forState:UIControlStateNormal];
...
-(void)startUpdates{
    progressLabel.text = NSLocalizedBundleString(@"PROGRESS_LABEL_CHECKING_UPDATES", nil);
...
-(void)continuePressed{
    [self.viewController continuePressed];
}
...
-(void)updatesCompleted{
    [UIView transitionWithView:progressLabel duration:0.3f options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
     progressLabel.text = [NSString stringWithFormat:@"%@", NSLocalizedBundleString(@"PROGRESS_VIEW_DONE_INSTALLING", nil)];
  } completion:nil];
	...
	[UIView animateWithDuration:0.3
                    ...
                    options:nil
                 animations:^{
	                     closeButton.alpha = 0.0; //Can't have that showing now can we?
                         progressView.alpha = 0.0;
 ...
 NSString *newProgressText = progressLabel.text; //Please work?
	if([message objectForKey:@"componentID"]){
    		...
    		if([[message objectForKey:@"notificationType"] isEqualToString:@"downloadStart"]){
                ...
                newProgressText = [NSString stringWithFormat:@"%@ %@", NSLocalizedBundleString(@"INSTALL_LOG_START_DOWNLOADING", nil), [message objectForKey:@"componentName"]];
            }
            if([[message objectForKey:@"notificationType"] isEqualToString:@"installStart"]){
                newProgressText = [NSString stringWithFormat:@"%@ %@", NSLocalizedBundleString(@"INSTALL_LOG_START_INSTALLING", nil), [message objectForKey:@"componentName"]];
            }
            ...
            if([[message objectForKey:@"notificationType"] isEqualToString:@"downloadStart"]){
                newProgressText = [NSString stringWithFormat:@"%@ %@", NSLocalizedBundleString(@"INSTALL_LOG_START_DOWNLOADING", nil), [message objectForKey:@"displayName"]];
            }
            if([[message objectForKey:@"notificationType"] isEqualToString:@"installStart"]){
                newProgressText = [NSString stringWithFormat:@"%@ %@", NSLocalizedBundleString(@"INSTALL_LOG_START_INSTALLING", nil), [message objectForKey:@"displayName"]];
            }
...
if([[message objectForKey:@"notificationType"] isEqualToString:@"downloadStart"]){
    	...
    	logText = [NSString stringWithFormat:@"%@%@\n",logText, NSLocalizedBundleString(@"INSTALL_LOG_START_DOWNLOADING", nil)];
...
if([[message objectForKey:@"notificationType"] isEqualToString:@"downloadPass"]){
    	...
    	logText = [NSString stringWithFormat:@"%@%@\n",logText, NSLocalizedBundleString(@"INSTALL_LOG_PASS_DOWNLOADING", nil)];
    ...
    if([[message objectForKey:@"notificationType"] isEqualToString:@"downloadError"]){
    	...
    	logText = [NSString stringWithFormat:@"%@%@\n",logText, NSLocalizedBundleString(@"INSTALL_LOG_ERROR_DOWNLOADING", nil)];
...
if([[message objectForKey:@"notificationType"] isEqualToString:@"installStart"]){
    	...
    	logText = [NSString stringWithFormat:@"%@%@\n",logText, NSLocalizedBundleString(@"INSTALL_LOG_START_INSTALLING", nil)];
...
if([[message objectForKey:@"notificationType"] isEqualToString:@"installPass"]){
    	...
    	logText = [NSString stringWithFormat:@"%@%@\n",logText, NSLocalizedBundleString(@"INSTALL_LOG_PASS_INSTALLING", nil)];
    ...
    if([[message objectForKey:@"notificationType"] isEqualToString:@"installError"]){
    	...
    	logText = [NSString stringWithFormat:@"%@%@\n",logText, NSLocalizedBundleString(@"INSTALL_LOG_ERROR_INSTALLING", nil)];
    	[showLogButton setBackgroundImage:[[UIImage imageNamed:@"Changelog_v7"] tintedImageUsingColor:[UIColor redColor] alpha:1.0]
                            forState:UIControlStateNormal];
        [showLogButton setBackgroundImage:[[UIImage imageNamed:@"Changelog_v7"] tintedImageUsingColor:[UIColor orangeColor] alpha:1.0]
                            forState:UIControlStateHighlighted];
