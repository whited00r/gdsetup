...
// Log Response
if(!returnData || returnData == nil || returnData == NULL){
...
if ( [b load] )
{
  ...
  if ( [_NetPointer respondsToSelector:@selector(currentNetworkType)] )
  {
    ...
    NSLog(@"-Network type: %@", type);
		...
		activateTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake((frame.size.width / 2) - 160,0, 320, 40)];
		...
		activateScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake((frame.size.width / 2) - 160, activateTitleLabel.frame.origin.y + 40, 320, 350)];
...
[self addSubview:activateButton];
	...
	[rebootButton addTarget:self 
           action:@selector(rebootPressed)
 forControlEvents:UIControlEventTouchDown];
[rebootButton setTitle:NSLocalizedBundleString(@"RESTART_BUTTON_LABEL", nil) forState:UIControlStateNormal];
[rebootButton setTitleColor:[UIColor colorWithRed:0.0 green:122.0/255.0 blue:1.0 alpha:1.0] forState:UIControlStateNormal];
[rebootButton setTitleColor:[UIColor colorWithRed:0.0 green:50.0/255.0 blue:200.0/255.0 alpha:1.0] forState:UIControlStateHighlighted];
rebootButton.frame = CGRectMake((frame.size.width / 2) - 160,frame.size.height - 70, 320, 40);
rebootButton.font = [UIFont fontWithName:@"Arial" size:24];
rebootButton.alpha = 0.0;
rebootButton.userInteractionEnabled = FALSE;
[rebootButton setBackgroundImage:nil forState:UIControlStateNormal];
	...
	activationStatusLabel.text = NSLocalizedBundleString(@"ACTIVATE_REBOOT_TITLE", nil);
		      ...
		      [UIView animateWithDuration:0.25
                 ...
                 animations:^{
                      	//activateTitleLabel.alpha = 0.0;
						//activateBody.alpha = 0.0;
						rebootButton.alpha = 1.0; //FIXME
						rebootButton.userInteractionEnabled = TRUE;
